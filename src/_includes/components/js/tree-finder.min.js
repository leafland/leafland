let treeWrapper = document.querySelector("#tree-wrapper"),
  ...
  clearCheckboxFilter = document.querySelector("#clear-checkbox-filter"),
  clearSelectFilter = document.querySelector("#clear-select-filter"),
  treeFinderStart = 0,
  treeFinderEnd = 11,
  imageDataSubset = [],
  inputsArray = document.querySelectorAll(".filter-input"),
    ...
    if (
      ((filteredData = trees.filter((tree) => {
        ...
        if (
          ...
          "" !== tree.tolerates &&
            void 0 !== tree.tolerates &&
            (compareValue += `${tree.tolerates}`),
          "" !== tree.winterFoliage &&
            void 0 !== tree.winterFoliage &&
            (compareValue += `${tree.winterFoliage}`),
          "" !== tree.origin &&
            void 0 !== tree.origin &&
            (compareValue += `${tree.origin}`),
          "" !== tree.soilType &&
            void 0 !== tree.soilType &&
            (compareValue += `${tree.soilType}`),
          "" !== tree.sunShade &&
            void 0 !== tree.sunShade &&
            (compareValue += `${tree.sunShade}`),
          "" !== tree.types &&
            void 0 !== tree.types &&
            (compareValue += `${tree.types}`),
          "" !== tree.height && void 0 !== tree.height)
        ) {
          let compareArray;
          tree.height.split(", ").forEach((value) => {
            compareValue += `${value}-h`;
          });
        }
        if ("" !== tree.width && void 0 !== tree.width) {
          let compareArray;
          tree.width.split(", ").forEach((value) => {
            compareValue += `${value}-w`;
          });
        }
        if ("" !== tree.floweringSeason && void 0 !== tree.floweringSeason) {
          let compareArray;
          tree.floweringSeason.split(", ").forEach((value) => {
            compareValue += `${value}-fls`;
          });
        }
        if ("" !== tree.flowerColor && void 0 !== tree.flowerColor) {
          let compareArray;
          tree.flowerColor.split(", ").forEach((value) => {
            compareValue += `${value}-fc`;
          });
        }
        if ("" !== tree.autumnColor && void 0 !== tree.autumnColor) {
          let compareArray;
          tree.autumnColor.split(", ").forEach((value) => {
            compareValue += `${value}-ac`;
          });
        }
        if ("" !== tree.foliageColor && void 0 !== tree.foliageColor) {
          let compareArray;
          tree.foliageColor.split(", ").forEach((value) => {
            compareValue += `${value}-flc`;
          });
        }
        if ("" !== tree.fruitingSeason && void 0 !== tree.fruitingSeason) {
          let compareArray;
          tree.fruitingSeason.split(", ").forEach((value) => {
        ...
        for (let i = 0; i < treeFilter.length; i++) {
          if (-1 !== treeFilter[i].search("-h"))
            for (let j = 0; j < heightsArray.length; j++) {
              if (-1 !== compareValue.search(heightsArray[j])) {
                heightFound = !0;
                break;
              }
              (heightFound = !1),
                j === heightsArray.length - 1 && (heightFound = !1);
            }
          else if (-1 !== treeFilter[i].search("-w"))
            for (let j = 0; j < widthsArray.length; j++) {
              if (-1 !== compareValue.search(widthsArray[j])) {
                widthFound = !0;
                break;
              }
              (widthFound = !1),
                j === widthsArray.length - 1 && (widthFound = !1);
            }
          else {
            if (-1 === compareValue.search(treeFilter[i])) {
              add = !1;
              break;
            }
            add = !0;
          }
          heightsArray.length > 0 && widthsArray.length > 0
            ? (add = !(!heightFound || !widthFound))
            : heightsArray.length > 0
            ? (add = !!heightFound)
            : widthsArray.length > 0 && (add = !!widthFound);
        }
        return add;
      })),
      ...
      for (
        let i = treeFinderStart;
        i < treeFinderEnd + 1 && !(i > filteredData.length - 1);
        i++
      )
        treeDataSubset.push(filteredData[i]);
      treeFinderEnd >= filteredData.length - 1
        ? (loadMoreButton.disabled = !0)
        : (loadMoreButton.disabled = !1);
    }
  } else if (0 !== trees.length) {
    for (
      let i = treeFinderStart;
      i < treeFinderEnd + 1 && !(i > trees.length - 1);
      i++
    )
      treeDataSubset.push(trees[i]);
    treeFinderEnd >= trees.length - 1
      ? (loadMoreButton.disabled = !0)
      : (loadMoreButton.disabled = !1);
  }
  if (0 === treeDataSubset.length) {
    ...
    (noProducts.id = "no-products"),
      ...
      (noProducts.textContent =
        "No trees found. Try different filter options."),
      treeWrapper.insertAdjacentElement("afterbegin", noProducts),
      (loadMoreButton.disabled = !0),
      (returnToTopButton.disabled = !0);
  } else {
    (returnToTopButton.disabled = !1),
      buttonDiv.style.setProperty("opacity", "1");
    for (let i = 0; i < treeDataSubset.length; i++)
      if (void 0 !== treeDataSubset[i] && null !== treeDataSubset[i]) {
...
function resetStartEnd() {
  (treeFinderStart = 0), (treeFinderEnd = 11);
}
window.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelector("select").addEventListener("input", (event) => {
    ...
    let selectData = Array.from(event.target.selectedOptions).reduce(
      (data, opt) => (
        data.push(opt.value),
        filterSettings[`${opt.dataset.category}`].includes(opt.value) ||
          (filterSettings[`${opt.dataset.category}`].push(opt.value),
          (filterSettings.empty = !1)),
        -1 !== opt.value.search("-h")
          ? heightsArray.push(opt.value)
          : -1 !== opt.value.search("-w") && widthsArray.push(opt.value),
        data
      ),
      []
    );
    selectData.length > 0
      ? !0 === clearSelectFilter.disabled && (clearSelectFilter.disabled = !1)
  ...
  inputsArray.forEach((input) => {
    input.addEventListener("input", () => {
      treeWrapper.style.setProperty("opacity", "0"),
        ...
        inputsArray.forEach((filterInput) => {
          !0 === filterInput.checked &&
            ...
            filterSettings[`${filterInput.dataset.category}`].includes(
              ...
              (filterSettings.empty = !1)),
            -1 !== filterInput.value.search("-h")
              ? heightsArray.push(filterInput.value)
              : -1 !== filterInput.value.search("-w") &&
        ...
        treeFilter.length > 0
          ? !0 === clearCheckboxFilter.disabled &&
            (clearCheckboxFilter.disabled = !1)
          : (clearCheckboxFilter.disabled = !0),
        (treeWrapper.innerHTML = ""),
  ...
  clearCheckboxFilter.addEventListener("click", () => {
    treeWrapper.style.setProperty("opacity", "0"),
      ...
      (widthsArray = []),
      inputsArray.forEach((filterInput) => {
        !0 === filterInput.checked && (filterInput.checked = !1);
      }),
      (clearCheckboxFilter.disabled = !0),
      (document.querySelector("#uses").textContent = "0"),
      (document.querySelector("#tolerates").textContent = "0"),
  ...
  clearSelectFilter.addEventListener("click", () => {
    ...
    for (let i = 0; i < options.length; i++)
      (options[i].selected = !1), (options[i].value = "");
    (clearSelectFilter.disabled = !0),
      (filterSettings = {
        empty: !0,
        uses: [],
