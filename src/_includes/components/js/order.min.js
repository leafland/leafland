let orderContent = document.querySelector("#order-content"),
  openOrder = document.querySelector("#open-order"),
  closeOrder = document.querySelector("#close-order"),
  submitOrder = document.querySelector("#submit-order"),
  totalCostText = document.querySelector("#order-overlay-total"),
...
function displayEmptyOrder() {
  orderContent.contains(productList) && orderContent.removeChild(productList),
...
async function updateOrder() {
  if (
    ...
    orderContent.contains(emptyOrder) && orderContent.removeChild(emptyOrder),
      ...
      orderTrees.forEach((tree) => {
        ...
        null !== tree.standardHeight.match(/\d+/g)
        ...
        (itemQuantity.type = "number"),
          ...
          itemQuantity.addEventListener("change", () => {
            parseInt(itemQuantity.value) < 1
              ? (itemQuantity.value = 1)
              : parseInt(itemQuantity.value) > parseInt(tree.maxQuantity) &&
                (itemQuantity.value = parseInt(tree.maxQuantity));
            for (let i = 0; i < orderTrees.length; i++)
              if (
                orderTrees[i].botanicalName === tree.botanicalName &&
                orderTrees[i].grade === itemQuantity.dataset.grade &&
                orderTrees[i].averageHeight === itemQuantity.dataset.height &&
            ...
            for (
              totalWholesaleCost = 0, totalRetailCost = 0, i = 0;
              i < orderTrees.length;
              i++
            )
              (totalWholesaleCost +=
            ...
            for (
              ...
              (totalWholesaleCost +=
                ...
                (totalRetailCost +=
                  orderTrees[i].quantity *
                  parseInt(orderTrees[i].retailPrice.slice(1), 10));
            updateStorage(), updateOrder(), window.dispatchEvent(orderUpdated);
          }),
...
openOrder.addEventListener("click", () => {
  ...
  window.addEventListener("storage", () => {
    "true" === localStorage.getItem("loggedIn")
      ? (loggedIn = !0)
      : (loggedIn = !1),
      updateOrder();
  }),
  ...
  window.addEventListener("loginUpdated", () => {
    !(async function init() {
      await getOrderImages(), updateOrder();
    })();
  });
