const submitForm = document.querySelector(".submit-form"),
  treesField = document.querySelector("#trees"),
  send = document.querySelector(".send-order"),
  treesDisplay = document.querySelector("#form-trees");
...
async function populateForm() {
  let totalFreight = 0,
    freightRegion = [],
    minimumCharge = "";
  "Northland" !== region.value &&
    "Manawatu" !== region.value &&
    "Gisborne" !== region.value &&
    "pickup" !== region.value.toLowerCase() &&
    freightData.forEach((datum) => {
      datum[0] === region.value &&
        (freightRegion.push({ grade: datum[1], price: datum[2] }),
        "Minimum" === datum[1] && (minimumCharge = datum[2]));
    }),
  ...
  title.classList.add("form-trees-title"), (title.textContent = "Trees:");
  let poaGrade = !1;
  submitOrderTrees.forEach((tree) => {
    ...
    if (
      ...
      for (i = 0; i < freightRegion.length; i++)
        if (-1 !== freightRegion[i].grade.search(tree.grade)) {
    ...
    averageHeight.innerHTML = `Average height: <span class="info-pill">${
      "n/a" === tree.averageHeight.toLowerCase()
        ? tree.averageHeight
    ...
    standardHeight.innerHTML = `Standard height: <span class="info-pill">${
      "none" === tree.standardHeight.toLowerCase()
        ? tree.standardHeight
    ...
    (treeImage.src = `https://leafland.imgix.net/images/trees/${tree.mainImage}?auto=format&w=300&q=75`),
      ...
      "Gisborne" !== region.value &&
      "Pickup" !== region.value
        ? "P.O.A" === freightPriceValue
          ? ((poaGrade = !0),
            (freightPrice.innerHTML =
              '<p>Freight per tree: <span class="info-pill">P.O.A</span></p>'),
        ...
        : ((freightPrice.innerHTML =
            '<p>Freight per tree: <span class="info-pill">N/A</span></p>'),
          (treesField.value += `<tr style="padding-bottom:3px;margin-bottom:3px;border-bottom:2px solid #000"><td><b>${tree.botanicalName.toUpperCase()} (${tree.commonName.toUpperCase()})</b></td> <td><b>${
            tree.grade
          }</b></td> <td><b>${
            "n/a" === tree.averageHeight.toLowerCase()
    ...
    "Pickup" !== region.value
      ? (poaGrade
          ? (totalFreight <= parseInt(minimumCharge.slice(1), 10)
            ...
            (orderTotal.innerHTML = `Order total: <span class="info-pill">$${
              -1 !== freightTotal.textContent.search("(Minimum freight charge)")
                ? (
      ...
      : ((freightTotal.innerHTML =
          'Freight total: <span class="info-pill">N/A</span>'),
...
(total = loggedIn ? totalWholesaleCost : totalRetailCost),
  ...
  submitForm.addEventListener("submit", (event) => {
    ...
    const {
      ...
      endpoint = "https://api.leafland.co.nz/default/send-order-email",
      body = JSON.stringify({
        name: name.value,
        email: email.value,
        phone: phone.value,
    ...
    !(async function () {
      await fetch(endpoint, requestOptions)
        ...
        (document.body.querySelector("#content").innerHTML =
          "<h1>Thanks for your order!</h1><p>From here, we will process your order and confirm we can supply the trees and grades you are after. We will also double-check the freight and let you know if there will be any extra costs (like a surcharge for rural addresses). Then we will send you an invoice that will have our bank details on for payment. We do not take payment via any sort of card.</p>");
    })();
  });
